...
//测试用
	...
	for (int i = 0; i < controlPointNums[0]; i++)
		for (int j = 3; j < controlPointNums[1]; j++)
			for (int k = 0; k < controlPointNums[2]; k++) {
				controlPoints[i][j][k].x = controlPoints[i][j][k].x*1.5 ;
				//controlPoints[i][j][k].y = controlPoints[i][j][k].y * 1.5;
				controlPoints[i][j][k].z = controlPoints[i][j][k].z * 1.5;
			}
*/
	...
	for (int i = 0; i < controlPointNums[0]; i++)
		for (int j = 0; j < controlPointNums[1]; j++)
			for (int k = 0; k < controlPointNums[2]; k++) {
				...
				if (j==2)
					controlPoints[i][j][k].x *=3.0f;

			}
				


	//controlPoints[3][4][2].y += 5;
}
void Bspinebody::draw(const Shader& shader) const {
	if (has_init) {
		shader.use();
        glUniform1f(glGetUniformLocation(shader.programId, "pointSize"), 1.0f);
        ...
        glUniform1f(glGetUniformLocation(shader.programId, "pointSize"), 25.0f);
		glBindVertexArray(VAOId[1]);
		//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

        //glPointSize(20.0f);opengles不支持
...
void Bspinebody::reSetupBuffer(){

	glBindVertexArray(this->VAOId[0]);
	glBindBuffer(GL_ARRAY_BUFFER, VBOId);
	glBufferSubData(GL_ARRAY_BUFFER,0, sizeof(glm::vec4) * 1000,controlPoints);
