...
public:
	...
	void loadComputeFile(const char* computePath)  {
		programId = 0;
		std::vector<ShaderFile> fileVec;
		fileVec.push_back(ShaderFile(GL_COMPUTE_SHADER, computePath));
		//fileVec.push_back(ShaderFile(GL_VERTEX_SHADER, computePath));
		loadFromFile(fileVec);
	}
	void loadShadingFile(const char* vertexPath,const char* fragPath){
		programId = 0;
		std::vector<ShaderFile> fileVec;
		fileVec.push_back(ShaderFile(GL_VERTEX_SHADER, vertexPath));
		fileVec.push_back(ShaderFile(GL_FRAGMENT_SHADER, fragPath));
	...
	void loadFromFile(std::vector<ShaderFile>& shaderFileVec)
	{
		...
		for (size_t i = 0; i < shaderCount; ++i)
		{
			...
			if (!loadShaderSource(shaderFileVec[i].filePath, shaderSource))
			{
				...
				LOGCATE("Error::Shader could not load file:%s", shaderFileVec[i].filePath);
				return;
			}
		...
		for (size_t i = 0; i < shaderCount; ++i)
		{
			...
			if (compileStatus == GL_FALSE) // 获取错误报告
			{
				...
				LOGCATE("Error::Shader file [%s] compiled failed,%s", shaderFileVec[i].filePath,&errLog[0]);
				bSuccess = false;
			}
		...
		if (bSuccess)
		{
			...
			if (linkStatus == GL_FALSE)
			{
				...
				LOGCATE("Error::shader link failed,%s", &errLog[0]);
			}
		}
